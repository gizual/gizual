...
export const RIGHTS_FD_DATASYNC = Math.trunc(1);
...
export class Iovec {
  ...
  static from_bytes(view: DataView, ptr: number): Iovec {
  ...
  static from_bytes_array(view: DataView, ptr: number, len: number): Array<Iovec> {
    const iovecs: Iovec[] = [];
    for (let i = 0; i < len; i++) {
      iovecs.push(Iovec.from_bytes(view, ptr + 8 * i));
    }
    return iovecs;
  }

  to_string(buffer: ArrayBufferLike) {
    const buf = new Uint8Array(buffer, this.buf, this.buf_len);
    return new TextDecoder().decode(buf);
  }

  set_string(buffer: ArrayBufferLike, str: string) {
    const buf = new Uint8Array(buffer, this.buf, this.buf_len);
...
export class Dirent {
  ...
  constructor(next_cookie: bigint, name: string, type: number) {
    const encoded_name = new TextEncoder("utf8").encode(name);
...
export const FDFLAGS_APPEND = Math.trunc(1);
...
export const EVENTRWFLAGS_FD_READWRITE_HANGUP = Math.trunc(1);

export const SUBCLOCKFLAGS_SUBSCRIPTION_CLOCK_ABSTIME = Math.trunc(1);
...
export const RIFLAGS_RECV_PEEK = Math.trunc(1);
export const RIFLAGS_RECV_WAITALL = 1 << 1;

export const ROFLAGS_RECV_DATA_TRUNCATED = Math.trunc(1);

export const SDFLAGS_RD = Math.trunc(1);
